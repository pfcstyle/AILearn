...
from transformers import CpmTokenizer  # pip install sentencepiece
...
def set_args():
    ...
    parser.add_argument('--vocab_path', default=os.path.join(work_dir, 'vocab/chinese_vocab.model'), type=str, required=False,
                        help='sp模型路径')
    parser.add_argument('--model_config', default=os.path.join(work_dir, 'config/cpm-small.json'), type=str, required=False,
                        help='需要从头训练一个模型时，模型参数的配置文件')
    parser.add_argument('--train_path', default=os.path.join(work_dir, 'data/train_novel.pkl'), type=str, required=False, help='经过预处理之后的数据存放路径')
    parser.add_argument('--max_len', default=200, type=int, required=False, help='训练时，输入数据的最大长度')

    parser.add_argument('--log_path', default=os.path.join(work_dir, 'log/train.log'), type=str, required=False, help='训练日志存放位置')
    ...
    parser.add_argument('--save_model_path', default='model', type=str, required=False,
                        help='模型输出路径')
    parser.add_argument('--pretrained_model', default=os.path.join(work_dir, 'model/novel_epoch40'), type=str, required=False,
                        help='预训练的模型的路径')
...
def main():
    ...
    # 创建模型
    if args.pretrained_model and os.path.exists(args.pretrained_model):  # 加载预训练模型

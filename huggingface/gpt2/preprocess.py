...
def preprocess():
    ...
    parser.add_argument('--vocab_file', default=os.path.join(work_dir, 'vocab/chinese_vocab.model'), type=str, required=False,
                        help='词表路径')
    parser.add_argument('--log_path', default=os.path.join(work_dir, 'log/preprocess.log'), type=str, required=False, help='日志存放位置')
    parser.add_argument('--data_path', default=os.path.join(work_dir, 'data/novel'), type=str, required=False, help='数据集存放位置')
    parser.add_argument('--save_path', default=os.path.join(work_dir, 'data/train_novel.pkl'), type=str, required=False, help='对训练数据集进行tokenize之后的数据存放位置')
    ...
    # 初始化tokenizer
    tokenizer = CpmTokenizer(vocab_file=args.vocab_file)#pip install jieba

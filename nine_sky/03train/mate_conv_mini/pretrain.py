...
data_path = os.path.join(root_path, 'data', 'nine_sky', 'Data')
pretrain_bin_path = os.path.join(data_path, 'seq-monkey', 'pretrain_data.bin')

def Logger(content):
    if not ddp or dist.get_rank() == 0:
        print(content)

# 到底采用什么学习率策略，可以通过观察前面几个epoch的loss变化来测试决定
def get_lr(it, all):
...
if __name__ == "__main__":
    ...
    # 可以使用offline模式，wandb.init(mode="offline")  
    # wandb sync wandb/run-20231001_123456-abcdefgh --project=pytorch-demo
    # wandb sync wandb/
    parser.add_argument("--use_wandb", action="store_true", help="Use Weights & Biases")
    parser.add_argument("--wandb_project", type=str, default="mate_conv_mini_3090", help="Weights & Biases project name")
    parser.add_argument("--num_workers", type=int, default=8, help="Number of workers for data loading")
    parser.add_argument("--data_path", type=str, default=pretrain_bin_path, help="Path to training data")
